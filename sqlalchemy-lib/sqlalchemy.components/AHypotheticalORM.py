...
class Root(Base):
    ...
    subs = relationship('Sub', backref=backref('root'), cascade='all, delete')    #  secondary=root_sub,
    # subs = relationship('Sub', back_populates='root')
    # def __init__(self, **kwargs):     # todo add variables to BaseClass init @BaseModel -> ****
    #     self.id = self._id
    #     self.var1 = kwargs.pop('var1', '')   # todo add <var> to Base&BaseModel
    #     self.var2 = kwargs.pop('var2', '')  # todo add <var> to Base&BaseModel
...
class Sub(Base):
    ...
    # root = relationship('Root', back_populates='subs')
    # def __init__(self, **kwargs):     # todo add variables to BaseClass init @BaseModel -> ****
    #     self.var1 = kwargs.pop('var1', '')   # todo add <var> to Base&BaseModel
    #     self.var2 = kwargs.pop('var2', '')  # todo add <var> to Base&BaseModel
